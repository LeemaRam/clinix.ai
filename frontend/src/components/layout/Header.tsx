...
const Header: React.FC<HeaderProps> = ({ onMenuToggle, isMobile, isSidebarCollapsed }) => {
  ...
  const pageMeta = useMemo(() => {
    const routes = [
      ...
      { path: '/analytics', title: t('navigation.analytics'), description: t('navigation.analyticsSubtitle') },
      { path: '/follow-ups', title: t('navigation.followUps'), description: t('navigation.followUpsSubtitle') },
      { path: '/appointments', title: t('navigation.appointments'), description: t('navigation.appointmentsSubtitle') },
              ...
              <div className="absolute right-0 mt-3 w-60 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft">
                ...
                <div className="py-2">
                  ...
                  <button 
                    ...
                    className="flex w-full items-center gap-3 px-4 py-3 text-sm text-error-600 transition-colors hover:bg-error-50"
                  >
                    <LogOut size={16} />
                    {t('auth.logout')}
                  </button>
